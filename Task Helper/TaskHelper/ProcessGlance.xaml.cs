...
namespace TaskHelper
{
    ...
    /// </summary>
    public sealed partial class ProcessGlance : Page
    {
        private NavigationHelper navigationHelper;
        private ObservableDictionary defaultViewModel = new ObservableDictionary();

        public ProcessGlance()
        {
        ...
        private void NavigationHelper_LoadState(object sender, LoadStateEventArgs e)
        {
            ...
            {
                int selection = Convert.ToInt32(e.NavigationParameter);
                DataContext = App.ViewModel.Procedures[selection];
                Step_List.DataContext =  App.ViewModel.Steps.Where(item => item.procedure_id == App.ViewModel.Procedures[selection].procedure_id).ToList();
                GroupInfoPanel.DataContext = App.ViewModel.Groups.Where(item => item.group_id == App.ViewModel.Procedures[selection].group_id).First();
            }
        }
		...
		{
			Step First = (Step_List.DataContext as List<Step>).FirstOrDefault();
			Frame.Navigate(typeof(FeedBack), First);
		}
	}
}
